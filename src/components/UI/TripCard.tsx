    ...
    >
        ...
        <div className="absolute top-4 right-4 z-20 group/badge">
          ...
          {trip.offers && (
            <div className="absolute top-full right-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-gray-100 p-4 opacity-0 translate-y-2 pointer-events-none group-hover/badge:opacity-100 group-hover/badge:translate-y-0 group-hover/badge:pointer-events-auto transition-all duration-300 ease-out z-30">
              ...
              <p className="text-[10px] font-bold text-brand-teal uppercase tracking-wider mb-2 border-b border-gray-100 pb-1">
                Included Deals
              </p>
              <ul className="space-y-2">
                {trip.offers.map((offer: any, idx:any) => (
                  <li
                    key={idx}
                    className="text-xs text-brand-navy font-medium flex items-start gap-2"
                  >
      ...
      <div className="p-6 flex flex-col grow">
        ...
        <div className="w-full overflow-hidden">
          <ul className="flex flex-wrap gap-3 items-center text-xs font-bold">
            {trip.features?.map((feature: any, idx: any) => (
              <li
                key={idx}
                className="flex items-center gap-2 bg-brand-green/50 border border-gray-300 rounded-full px-3 py-1.5 mb-2 max-w-full"
              >
        ...
        <div className="space-y-2 mb-6 bg-gray-50 p-4 rounded-xl">
          {trip.services?.map((service: any, idx: any) => (
            <div
              key={idx}
              className="flex items-center gap-3 text-sm text-gray-600"
            >
              <Check size={16} className="text-brand-green" strokeWidth={2} />
        ...
        {trip.tags && (
          <div className="w-full overflow-hidden mb-6 relative">
            ...
            >
              ...
              {[...trip.tags, ...trip.tags].map((tag: any, idx) => (
                <span
                  key={idx}
                  className="text-sm font-bold capitalize text-gray-400 italic hover:text-brand-coral transition-colors"
                >
                  #{tag?.name}
                </span>
              ))}
        ...
        <div className="mt-auto pt-6 border-t border-gray-100 flex items-center justify-between">
          <div className="flex flex-col">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-2xl font-black text-brand-navy">
                {trip.current_price?.toLocaleString("en-US", {
                  style: "currency",
                  currency: "USD",
                })}
              </span>
          ...
          <button className="px-6 py-2.5 rounded-full bg-white border-2 border-brand-coral text-brand-coral font-bold text-sm shadow-sm hover:bg-brand-coral hover:text-white hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
            View Details
          </button>
        </div>
      </div>
